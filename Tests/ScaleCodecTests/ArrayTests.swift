...
final class ArrayTests: XCTestCase {
    ...
    func testFixedArray() {
        ...
        do {
            let data = try ScaleCodec.encode(value, .fixed(5))
            XCTAssertEqual(data.hex, encoded)
            let decoded = try ScaleCodec.decode(Array<UInt16>.self, .fixed(5), from: data)
            XCTAssertEqual(decoded, value)
        } catch { XCTFail("\(error)") }
        XCTAssertThrowsError(try ScaleCodec.encode(value, .fixed(3)))
        XCTAssertThrowsError(try ScaleCodec.decode(Array<UInt16>.self, .fixed(6), from: encoded.hexData!))
    }
}
